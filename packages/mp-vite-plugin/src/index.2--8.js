...
function mpVitePlugin(options) {
    ...
    return {
          ...
          await new Promise((resolve)=>{
              ...
              for (const entryName of entryNames) {
                ...
                for (const [fileName, chunkOrAsset] of Object.entries(bundle)) {
                  ...
                  // TODO: 入口文件中没有bundle为css的文件，是否是兼容其他配置？？ 要确保所有的css都经过了处理
                  // 调整 css 内容
              ...
              for (const entryName of entryNames) {
                  ...
                  // 页面 js
                  // 这里准备好注入到小程序page中，注入一个代码执行的方法，接受小程序创建的window和document
      ...
      // 服务给 generate.autoBuildNpm 自动将小程序新增的两个运行时依赖加入，在generateBundle会在dist下放入package.json
      async closeBundle() {
        // TODO: 依赖安装失败
        // 处理自动安装小程序依赖
